...
function NavBar() {
  ...
  const tutorItems = [{ text: "Gestion tutorias", link: "/managementTutoring" }];
    ...
    <div className="fixed top-0 w-full z-10 shadow-md">
      {session?.user ? (
        <Navbar>
          ...
          <NavbarMenu>
          {session.user.role === 'ADMINISTRATOR' && adminItems.map((item, index) => (
              <NavbarItem key={index}>
            ...
            {session.user.role === 'TUTOR' && tutorItems.map((item, index) => (
              <NavbarItem key={index}>
                <Link color="foreground" href={item.link} onClick={toggleMenu}>
                  {item.text}
                </Link>
